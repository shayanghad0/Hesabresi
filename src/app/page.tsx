  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`w-10 h-10 rounded-lg bg-gradient-to-br ${colorMap[color] || colorMap.blue} flex items-center justify-center text-white`}
        >
          {icon}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <StatCard title="درآمد امروز" value={formatCurrency(data.todayIncome)} icon={<TrendUpIcon size={20} />} color="green" />
        <StatCard title="هزینه امروز" value={formatCurrency(data.todayExpense)} icon={<TrendDownIcon size={20} />} color="red" />
        <StatCard title="درآمد ماهانه" value={formatCurrency(data.monthlyIncome)} icon={<WalletIcon size={20} />} color="blue" />
        <StatCard title="هزینه ماهانه" value={formatCurrency(data.monthlyExpense)} icon={<MoneyFlyIcon size={20} />} color="yellow" />
        <StatCard title="کل درآمد" value={formatCurrency(data.totalIncome)} icon={<BankIcon size={20} />} color="green" />
        <StatCard title="کل هزینه" value={formatCurrency(data.totalExpense)} icon={<ReceiptIcon size={20} />} color="red" />
        <StatCard title="سود" value={formatCurrency(data.profit)} icon={<ChartBarIcon size={20} />} color="cyan" />
        {data.loss > 0 && <StatCard title="زیان" value={formatCurrency(data.loss)} icon={<WarningIcon size={20} />} color="red" />}
        <StatCard title="تعداد کارکنان" value={formatNum(data.employeeCount)} icon={<UsersIcon size={20} />} color="purple" />
        <StatCard title="حقوق پرداختی" value={formatCurrency(data.totalSalaryPaid)} icon={<CreditCardIcon size={20} />} color="indigo" />
        <StatCard title="مجموع مالیات" value={formatCurrency(data.totalTax)} icon={<BuildingIcon size={20} />} color="pink" />
        <StatCard title="پاداش‌ها" value={formatCurrency(data.totalBonuses)} icon={<GiftIcon size={20} />} color="yellow" />
      </div>

      {/* Charts */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
          <h3 className="font-bold text-gray-800 dark:text-white mb-4"><ChartBarIcon size={16} className="inline mr-1.5 align-middle" /> درآمد و هزینه ماهانه</h3>
          <BarChart
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
          <h3 className="font-bold text-gray-800 dark:text-white mb-4"><TrendUpIcon size={16} className="inline mr-1.5 align-middle" /> روند سود</h3>
          <LineChart
            data={data.monthlyChartData.map((m) => ({
              label: m.month,
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex gap-2">
          ...
          <Button variant="secondary" onClick={handleExport}><DownloadIcon size={16} className="inline ml-1" /> خروجی اکسل</Button>
        </div>
        <input
          type="text"
          placeholder="جستجو..."
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex gap-2">
          ...
          <Button variant="secondary" onClick={() => window.open("/api/export?type=income", "_blank")}><DownloadIcon size={16} className="inline ml-1" /> خروجی اکسل</Button>
        </div>
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex gap-2">
          ...
          <Button variant="secondary" onClick={() => window.open("/api/export?type=expenses", "_blank")}><DownloadIcon size={16} className="inline ml-1" /> خروجی اکسل</Button>
        </div>
        <div className="flex items-center gap-3">
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex gap-2">
          ...
          <Button variant="secondary" onClick={() => window.open("/api/export?type=salary", "_blank")}><DownloadIcon size={16} className="inline ml-1" /> خروجی اکسل</Button>
        </div>
      </div>
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex gap-2">
          ...
          <Button variant="secondary" onClick={() => window.open("/api/export?type=bonuses", "_blank")}><DownloadIcon size={16} className="inline ml-1" /> خروجی اکسل</Button>
        </div>
      </div>
...
<div class="grid">
<div class="card green"><div class="card-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="5" width="22" height="15" rx="3" /><circle cx="18" cy="13" r="1.5" /></svg></div><div class="card-title">کل درآمد</div><div class="card-value">${s.totalIncome.toLocaleString("fa-IR")} ریال</div></div>
<div class="card red"><div class="card-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="6" width="18" height="12" rx="2" /><circle cx="10" cy="12" r="2.5" /><path d="M19 9l3-2m0 0l-3-2m3 2h-3" /><path d="M19 15l3 2m0 0l-3 2m3-2h-3" /></svg></div><div class="card-title">کل هزینه</div><div class="card-value">${s.totalExpense.toLocaleString("fa-IR")} ریال</div></div>
<div class="card cyan"><div class="card-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><line x1="3" y1="20" x2="3" y2="4" /><line x1="8" y1="20" x2="8" y2="9" /><line x1="13" y1="20" x2="13" y2="13" /><line x1="18" y1="20" x2="18" y2="6" /><line x1="1" y1="20" x2="21" y2="20" /></svg></div><div class="card-title">سود خالص</div><div class="card-value">${s.netProfit.toLocaleString("fa-IR")} ریال</div></div>
<div class="card purple"><div class="card-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 2 7 22 7" /><line x1="2" y1="7" x2="22" y2="7" /><line x1="6" y1="22" x2="6" y2="11" /><line x1="12" y1="22" x2="12" y2="11" /><line x1="18" y1="22" x2="18" y2="11" /><polygon points="2 11 12 22 22 11" /></svg></div><div class="card-title">مجموع مالیات</div><div class="card-value">${s.totalTax.toLocaleString("fa-IR")} ریال</div></div>
<div class="card indigo"><div class="card-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" /><line x1="1" y1="10" x2="23" y2="10" /><line x1="6" y1="15" x2="10" y2="15" /></svg></div><div class="card-title">حقوق پرداختی</div><div class="card-value">${s.totalSalaryPaid.toLocaleString("fa-IR")} ریال</div></div>
<div class="card yellow"><div class="card-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="10" width="18" height="12" rx="2" /><rect x="3" y="4" width="18" height="4" rx="2" /><line x1="12" y1="4" x2="12" y2="22" /><path d="M12 4c-1-2-3-2-4 0s1 3 4 0" /><path d="M12 4c1-2 3-2 4 0s-1 3-4 0" /></svg></div><div class="card-title">پاداش‌ها</div><div class="card-value">${s.totalBonuses.toLocaleString("fa-IR")} ریال</div></div>
</div>
    ...
    <div className="max-w-2xl">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        ...
        <div className="mt-6 flex items-center gap-3">
          ...
          {saved && (
            <span className="text-green-600 dark:text-green-400 text-sm"><span className="flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width={14} height={14} fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg> تنظیمات با موفقیت ذخیره شد</span></span>
          )}
        </div>
      </div>
